---
description: Generate all preparation files required for SDD workflow for brownfield
  projects (existing projects), including constitution.md, template files, etc., enabling
  projects to immediately enter spec-driven development mode.
handoffs:
...
<!-- Extension: brownfield-bootstrap -->
<!-- Config: .specify/extensions/brownfield-bootstrap/ -->
## User Input
...
```
[Project Root]/
├── .specify/                          # SDD configuration directory (toolchain depends on this path)
│   ├── memory/
...
│   └── scripts/                       # Workflow scripts (optional, included in full installation)
│       ├── bash/
│       └── pwsh/
└── specs/                             # Feature spec working directory
    └── [###-feature-name]/            # Organized by feature branch
        ├── spec.md
        ├── plan.md
        └── tasks.md
```
...
### 🚨 Mandatory Pre-Output Self-Check List

**Before outputting any files, must confirm all following checks pass:**

| # | Check Item | Correct ✓ | Wrong ✗ | Verification Method |
|---|------------|-----------|---------|---------------------|
...
**❌ If any check fails, must correct before output**
...
1. **Extract from config files**:
    - ESLint/Prettier config → JavaScript/TypeScript standards
...
#### Step 0.5: Multi-Module/Multi-Subproject Analysis ⚠️ Critical
...
│   ├── [order-api]
│   ├── [order-dao]
│   ├── [user-api]  ← Cross-module dependency
│   └── [common]
└── [common]
```
...
- **Allowed dependency direction**: Upper layer → Lower layer (service → api → common)
- **Forbidden dependency direction**: Lower layer → Upper layer, circular dependencies
...
1. **Is this an externally exposed interface/DTO?**
   - Yes → Place in corresponding domain's `*-api` module
   - No → Continue

2. **Is this business logic/service implementation?**
   - Yes → Place in corresponding domain's `*-service` module
   - No → Continue

3. **Is this a database entity/data access?**
   - Yes → Place in corresponding domain's `*-dao` module
   - No → Continue
...
5. **Is this a new business domain?**
   - Yes → Consider creating new module group (xxx-api, xxx-service, xxx-dao)
...
3. **🔵 Continuous refactoring**: Refactor after test passes, keep code clean
...
**Dependency Rules**:
- Allowed: [Upper layer module] → [Lower layer module]
...
<!--
  ...
  Each user story/journey must be independently testable—meaning even if only one is implemented,
...
⚠️ **The following tech stack is locked by project constitution, cannot be changed**:
...
│   └── [New files, using project naming conventions]
└── [Test directory]
    └── [New test files]
```
...
⚠️ **The following paths are locked by project constitution**:
...
**⚠️ Critical**: Only add necessary new dependencies, prefer reusing existing modules
...
- [ ] T007 **Run tests, confirm failure** ← 🔴 Must fail first
...
- [ ] T010 **Run tests, confirm passing** ← 🟢 Must pass

### 🔵 TDD Phase 3: Refactor

- [ ] T011 Refactor code, improve structure
- [ ] T012 **Run tests, confirm still passing** ← 🔵 Must not break
...
1. **Constitution↔Template Consistency**:
...
2. **Template↔Template Consistency**:
...
3. **Template↔Project Consistency**:
...
**Bootstrap Status**: ✅ Success / ⚠️ Manual Intervention Needed / ❌ Failed

### 🚨 Output File Final Verification *(All must pass)*

| File | Expected Path | Actual Path | Verification Result |
|------|---------------|-------------|---------------------|
| Project Constitution | `.specify/memory/constitution.md` | [Fill actual path] | ✅/❌ |
| Spec Template | `.specify/templates/spec-template.md` | [Fill actual path] | ✅/❌ |
| Plan Template | `.specify/templates/plan-template.md` | [Fill actual path] | ✅/❌ |
| Tasks Template | `.specify/templates/tasks-template.md` | [Fill actual path] | ✅/❌ |

**⚠️ If any ❌, must correct and re-output that file**

### Generated File List

| File | Path | Status | Notes |
|------|------|--------|-------|
| Project Constitution | `.specify/memory/constitution.md` | ✅ | Core file |
| Spec Template | `.specify/templates/spec-template.md` | ✅ | |
| Plan Template | `.specify/templates/plan-template.md` | ✅ | |
| Tasks Template | `.specify/templates/tasks-template.md` | ✅ | |
| Specs Directory | `specs/` | ✅ | Empty directory |
...
## 🧪 TDD Test-Driven Development Discipline *(Mandatory)*
...
├─────────────────────────────────────────────────────────────┤
│  1. 🔴 RED    │ Write failing test → Run tests → Confirm failure     │
│  2. 🟢 GREEN  │ Write minimum code to pass → Run tests → Confirm pass │
│  3. 🔵 REFACTOR │ Refactor code → Run tests → Confirm still passing  │
│  4. 🔁 REPEAT │ Return to step 1, process next test case             │
└─────────────────────────────────────────────────────────────┘
```
...
```
Spec → Plan → Tasks → [TDD Cycle] → Complete
                │
┌───────────────┴───────────────┐
│  Within each task:            │
│  T1: Write test case (🔴 expect failure)   │
│  T2: Run test, confirm failure │
│  T3: Implement feature code (🟢 minimal implementation) │
│  T4: Run test, confirm pass    │
│  T5: Refactor and optimize (🔵 keep tests passing) │
└───────────────────────────────┘
```
...
- [ ] New code test coverage ≥ 80% (core business logic 100%)
